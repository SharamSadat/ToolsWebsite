...
function NonElectric() {
  const [items, setItems] = useState([]);

  useEffect(() => {
    const stored = JSON.parse(localStorage.getItem("items")) || [];
    const filtered = stored.filter((item) => item.category === "non-electronic");
    setItems(filtered);
  }, []);
  ...
  return (
    <>
      <ToolsNavbar />
      <h1 className={`section-title relative top-12 mt-24 ${darkMode ? '!text-orange-400' : '!text-orange-500'}`}>{t.no_electric}</h1>
